...
if [[ $FLAVOR == *"dev"* ]]; then
    ...
    cp $PRODUCT_NAME/Firebase/dev/firebase_app_id_file.json $SRCROOT/firebase_app_id_file.json
elif [[ $FLAVOR == *"prod"* ]]; then
    cp $PRODUCT_NAME/Firebase/prod/GoogleService-Info.plist $PRODUCT_NAME/GoogleService-Info.plist
    cp $PRODUCT_NAME/Firebase/prod/firebase_app_id_file.json $SRCROOT/firebase_app_id_file.json
else
